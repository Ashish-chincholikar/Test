...
load_button = tk.Button(root, text="Load Dataset", command=lambda: load_dataset(filedialog.askopenfilename(filetypes=[("CSV files", "*.csv")]), ""))
...
fp_growth_button = tk.Button(root, text="FP-Growth", command=run_fp_growth_algorithm)
fp_growth_button.grid(row=1, column=1, padx=5, pady=5)

# Create a button to display the dataset
display_button = tk.Button(root, text="Display Dataset", command=display_dataset)
...
load_reduced_button = tk.Button(root, text="Reduce Dataset", command=load_reduced_dataset)
load_reduced_button.grid(row=0, column=1, padx=5, pady=5)

load_cleaned_button = tk.Button(root, text="Clean Dataset", command=load_cleaned_dataset)
load_cleaned_button.grid(row=0, column=2, padx=5, pady=5)

load_transformed_button = tk.Button(root, text="Transformed Dataset", command=load_transformed_dataset)
load_transformed_button.grid(row=0, column=3, padx=5, pady=5)

# Create a button to load the final preprocessed dataset
load_final_preprocessed_button = tk.Button(root, text="Final Preprocess Dataset", command=load_final_preprocessed_dataset)
load_final_preprocessed_button.grid(row=0, column=4, padx=5, pady=5)

# Create a button to exit the application
exit_button = tk.Button(root, text="Exit", command=exit_application)
exit_button.grid(row=1, column=6, padx=5, pady=5)
